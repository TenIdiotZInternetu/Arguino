...
\begin{enumerate}
    \item Configure the CMake project in \texttt{backend/} with a \texttt{-DSKETCH} flag specifying the path to the sketch directory. This has to be done only when changing the sketch.
    
    \item Build the configured CMake directory after any change to the sketch. Available targets are:
    
    \begin{description}
        \item[\texttt{arguino}] for IPC over shared memory (preferred).
        \item[\texttt{arguino-tcp}] for IPC over TCP connection
    \end{description}

    \item The executable file will have the format \texttt{<build target>-<sketch name>}
\end{enumerate}

For example, to run the \texttt{wave} sketch, you can execute these commands from the root of the repository: \footnotemark

\begin{lstlisting}[language=shell, caption={Example build process of the backend}]
    cmake -S #./backend# -B .#/backend/build# #-DSKETCH=tests/wave#
    cmake --build #./backend/build# --target #arguino#
    ./backend/build/executable/arguino-wave
\end{lstlisting}

\footnotetext{The path of the actual executable may change depending on the build tool. The example shows usage with Linux and Clang. On Windows, the \texttt{.exe} extension is usually appended, i.e. \texttt{.../<target>-<sketch>.exe}.}
...
\subsubsection{GUI controls}

\begin{description}
  \item[\texttt{Left mouse button}] Interact with controls (e.g. buttons)
  \item[\texttt{Mouse wheel scroll}] Zoom and unzoom canvas
  \item[\texttt{Mouse wheel press}] Pan canvas
  \item[\texttt{Holding Alt key}] Hold mode. All controls clicked on will remain in a pressed state when clicked, until the control is clicked again. For example when holding multiple buttons is required.
\end{description}
...
\begin{description}
    ...
    \item[\texttt{-{}-shmem-size <int>}] Size of the memory-mapped region per circular buffer, specified in pages.{\footnotemark} [\texttt{1}]
\end{description}

\footnotetext{On Linux and Windows, one page is usually 4096B.}
...
\footnotetext{The actual implementations pad the numbers in such a manner, that all messages of the same event type have the same length. }
