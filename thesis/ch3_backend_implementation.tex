...
The singleton's instance is created on the heap within the static \texttt{init} and its pointer stored in the \texttt{s\_instance} member. The \texttt{eventCallback} paramater of this method must be set to the handler, that should be called upon event consumption. In this our implementation that is the \texttt{on\_event} function from \texttt{ipcAdapter.hpp}. This callback is stored into \texttt{f\_eventCallback}.

\texttt{state} and \texttt{queue} methods are used to access the objects owned by the \texttt{Simulator}. But queue is more typically accessed through the \texttt{handle\_event} and \texttt{handle\_events} calls. They both consume all remote events waiting in the queue, but the \texttt{handle\_event} variant also puts another event in the queue before starting the consumption.

The \texttt{Simulator} also owns a \texttt{Timer}, this one representing the real time passed. At the moment, both timers show roughly the same time, but it is anticipated, that the simulation timer could be manipulated by the user in the future versions. Lastly, the \texttt{Simulator} class holds a shared pointer to a logger. It also exposes shortcut methods for passing log messages of different verbosity to the logger. This logger is used for any for messages unrelated to the IPC.
...
The members of the \texttt{Event} struct mostly match the event specification in subsections \ref{subsec:events}. The additions are the \texttt{id} and \texttt{action} members. We already justified the use \texttt{id} member in the previous section. It is assigned in the constructor from the static \texttt{s\_nextEventId}, which is then incremented. The \texttt{action} is assigned as a lambda function from within the static factory methods. This lambda is executed during the \texttt{EventQueue}'s \texttt{execute\_next\_event} call.
...
At this point, the LVT of the event is not known. That is the responsibility of the scheduling entity. For local events it is the \texttt{EventQueue}, for remote events it is IPC, which has to decode LVT from incoming message.
