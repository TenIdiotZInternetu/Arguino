...
Our humble GUI consists mostly of the circuit canvas. A zoomable and pannable space where the components of a circuit can be placed to specific coordinates, uncostrained by the usual layout structures of Avalonia. Each of the component types have its own set of SVG images that are drawn to the canvas. Each individual component always references one of the SVGs from this set, representing its current state to the user. 

The preferred architecture for Avalonia applications is \textit{MVVM}, or \textit{Model-View-ViewModel}. \textit{View} is the visual layout of the window and its parts. It is written in \textit{XAML} (Extensible Application Markup Language) in files with the \texttt{.axaml} extension. It binds to data and events in the \textit{viewmodel}, which is responsible for behavior of what is being shown. \cite{avalonia-mvvm}
